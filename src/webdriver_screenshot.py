...
class WebDriverScreenshot:
    ...
    def __get_correct_height(self, url, width=1920):
        chrome_options=self.__get_default_chrome_options()
        chrome_options.add_argument('--window-size={}x{}'.format(width, 1080))
    ...
    def take_screenshot(self, url, filename, selectors, width, height):
        #if height is None: 
        ...
        if len(selectors) is 0:
            ...
            for s in selectors: 
                logger.info('Processing selector {}', s)
                if s['type']=='class':
                    element = driver.find_element_by_class_name(s['value'])
                    element.screenshot(filename)
                    logger.info("SCREENSHOT TAKEN")
                #TODO: Add rest of selectors that should be supported
